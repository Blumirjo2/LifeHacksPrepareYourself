...
function stringMasLarga(strings) {
  ...
  // Tu código aca
  let palabras = strings;
    let totalDeCaracteres  = [];
    for(let palabra of palabras) {
      totalDeCaracteres.push(palabra.length);
    }
    
    let maximus = Math.max.apply(null, totalDeCaracteres);
    
    for (let elemento of palabras) {
      if (elemento.length === maximus) {
            return elemento;
      }
    }
}
...
function sumArray(array, n) {
  ...
  // Tu código aca:
   const tamañoJK = array.length;

  for (let i = 0; i < tamañoJK; i++) {

    const a = array[i];

    for (let j = 0; j < tamañoJK; j++) {
...
function pluck(array, propiedad) {
  ...
  // Tu código acá:
  var devolucionDeValorDePropiedad = array.map((a) => a[propiedad]);

    return devolucionDeValorDePropiedad; 
  }
...
function crearClasePersona() {
  class Persona {
    ...
    getPromedioEdad() {
      ...
      let arrT = [];
      for (var i = 0; i < this.amigos.length; i++) {
             arrT.push(this.amigos[i]["edad"]);
      }
      let suma = (arrT[0] + arrT[1]) / arrT.length;
      return suma;
     }
